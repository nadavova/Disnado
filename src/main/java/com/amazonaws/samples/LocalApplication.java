...
public class LocalApplication {
	...
	public static void main(String[] args) {
	    ...
	    //queue
		 ...
		 Instance managerInstance = createManagerInstance(Integer.parseInt(args[args.length - 1]));
			...
			//Send messages to the manager about the new uploaded files        
	        ...
	        numberOfFiles = args.length - args.length/2 - 1;
			while(numberOfFiles > 0) {
	            ...
	            for(Message message : sqs.receiveMessage(receiveMessageRequest).getMessages()) {
	            	...
	            	if(message == null)
		                ...
		                //if(!htmlExport(object)) // file that belong to other local
		                	//continue;
	...
	private static void uploadFiles(AmazonS3 s3, String[] args) {     
        ...
        //for(int i = 0; i < (args.length - 1)/2; i++) {
        	file = new File(args[0]);//i 
            //key = file.getName().replace('\\', '_').replace('/','_').replace(':', '_');//why?
        	key = file.getName();
        ...
        /*
        file = new File("C:\\Users\\user\\Desktop\\manager.jar"); 
        key = file.getName().replace('\\', '_').replace('/','_').replace(':', '_');
        PutObjectRequest req = new PutObjectRequest(bucketName, key, file);
        s3.putObject(req);
        /*
        file = new File("C:\\Users\\user\\Desktop\\worker.jar"); 
        key = file.getName().replace('\\', '_').replace('/','_').replace(':', '_');
        req = new PutObjectRequest(bucketName, key, file);
        s3.putObject(req);*/
	}
